...
from src.core.dependencies import get_current_user, require_team_role_direct
...
async def create_team(
    ...
    return success_response(result.model_dump(), "创建成功")
...
async def get_teams(
    ...
    return success_response([team.model_dump() for team in result], "获取成功")
...
async def get_team(
    ...
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    _ = Depends(require_team_role_direct("viewer"))
):
    team_service = TeamService(db)
...
async def update_team(
    ...
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    _ = Depends(require_team_role_direct("admin"))
):
    team_service = TeamService(db)
...
async def delete_team(
    ...
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    _ = Depends(require_team_role_direct("owner"))
):
    team_service = TeamService(db)
...
async def add_team_member(
    ...
    return success_response(result.model_dump(), "添加成功")
...
async def transfer_ownership(
    ...
    return success_response(None, "转让成功")
